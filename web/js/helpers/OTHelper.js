    ...
    .then((devices) => {
      ...
      if (videoIndex === 1 && oldTrack) {
        ...
        for (let i = 0; i < vidDevices.length; i += 1) {
          if (vidDevices[i].label !== oldTrack.label) {
            videoIndex = i;
            break;
          }
        }
      ...
      return getUserMediaHelper().catch(userMediaError);
    })
    .then((newVideoStream) => {
      ...
      return getAllPeerConnections()
        .then(pcs =>
          Promise.all(pcs.map(pc => pc.findAndReplaceTrack(oldTrack, newTrack)))
        )
        .then(() => {
